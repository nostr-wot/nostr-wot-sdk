...
## @nostr-wot/graph 0.3.0 / nostr-wot-sdk 1.0.2 — 2026-09-20
...
- **`PoolLike` interface** now uses `any` for filter/event parameters so `SimplePool` from nostr-tools is directly assignable without casting
...
- **CI build failure** — Added `esbuild-plugin-solid` to `devDependencies` so it's available in CI environments
